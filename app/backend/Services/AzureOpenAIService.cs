...
public class AzureOpenAIService
{
    ...
    You're an AI assistant that will join a conference call. If someone ask you to join a conference, just say "Ok".
    """;
        ...
        try
        {
            ...
            await foreach (ConversationUpdate update in m_aiSession.ReceiveUpdatesAsync(m_cts.Token))
            {
                ...
                if (update is ConversationInputTranscriptionFinishedUpdate transcriptionCompletedUpdate)
                {
                    ...
                    if (transcriptionCompletedUpdate.Transcript.Contains(" silence ", StringComparison.InvariantCultureIgnoreCase))
                    {
                        Console.WriteLine("  -- Detected silence, stopping audio");
                        StopAudio();
                    }
                }
